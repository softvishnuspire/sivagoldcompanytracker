...
export default function CompletedCasesPage() {
  ...
  return (
    <div className="space-y-6 animate-fadeIn">
      <div>
        <h1 className="text-3xl font-extrabold tracking-tight text-white">Completed Cases</h1>
        <p className="text-slate-400 text-sm mt-1">Review finalized gold buyouts and closed cases ledger details.</p>
      </div>
              ...
              <tbody className="divide-y divide-amber-500/5 text-sm">
                {leads.map((lead) => (
                  <tr key={lead.id} className="hover:bg-[#471a15]/20 transition-colors">
                    ...
                    <td className="px-6 py-4 font-mono text-slate-300 text-xs">{lead.gold_weight ? `${lead.gold_weight}g` : 'N/A'}</td>
                    <td className="px-6 py-4 font-mono text-slate-300 text-xs">₹{Number(lead.loan_amount || 0).toLocaleString('en-IN')}</td>
