...
- **Removed MD Funds Approval Simulation**: Completely removed the "SIMULATE MD FUNDS APPROVAL" button and its underlying handler function `handleSimulateFundsApproval` from the Field Executive Lead Details workflow view (`frontend/src/app/executive/lead/[id]/page.tsx`) to ensure status transitions rely exclusively on the actual MD panel authorization flow.
- **File Upload Validation & Magic Bytes Verification**: Secured the application against malicious file uploads. Enforced file extension and size validation (10MB limit) on both Telecaller and Executive frontends, adding native `accept` attributes to file input elements. Added server-side MIME type filtering via Multer and implemented robust binary magic bytes verification to inspect file headers on the backend, checking for JPEG, PNG, and PDF signatures for both multipart/form-data and Base64 inline documents.
...
- **RM Dashboard Mock Data Removal**: Removed all remaining mock and static data from the Regional Manager (RM) Dashboard workspace:
  - Made the header date display dynamic, replacing the hardcoded '02 June 2026, Monday'.
  - Initialized the profile state with clean, empty parameters to prevent placeholder details showing before the API fetch completes.
  - Dynamically computed avatar initials from the loaded profile object rather than using static 'RK'.
  - Discarded a duplicate IIFE donut chart element inside the 'Lead Status Summary' container.
  - Linked the 'Executives' register view to render real DB status and database employee codes rather than indices and hardcoded 'Active' labels. Updated the backend RM executives endpoint to fetch `employee_code`.
  - Upgraded the 'Lead Sources' tab to dynamically compute lead volume counts per source from database leads rather than hardcoding static mock listings. Added a 'Total Leads' count column to the view.
- **MD TypeScript Compiler Fix**: Resolved an implicit `any` type compilation error (`Parameter 'd' implicitly has an 'any' type`) in the documents search callback inside [md/page.tsx](file:///c:/sivagoldcompanytracker/frontend/src/app/md/page.tsx#L1070) by explicitly annotating the parameter `d` with the type `any` (i.e. `(d: any)`).
- **MD Fund Approval Loan Amount Sync**: Fixed a bug where when the MD accepted a fund request and modified the requested amount, the new approved amount was not saved in the `leads.loan_amount` column. Updated the MD fund approval endpoint (`POST /api/md/fund-approval`) and the executive status update endpoint (`POST /api/executive/update-status`) to persist the approved fund amount to `leads.loan_amount` so the Field Executive dashboard receives and pre-fills the correct approved value.
- **Global UI/UX Refinement System**: Refactored the entire Shiva Gold application to provide professional, enterprise-grade UX:
  - Installed and configured `@tanstack/react-query`, `sonner`, `nprogress`, and `react-loading-skeleton` globally inside a root `ClientProviders.tsx` layout wrapper.
  - Implemented an `OfflineIndicator` floating alert and network speeds checker at the root level.
  - Hooked `nprogress` to start on API fetches and route transitions using programmatic counter handles in `api.ts`.
  - Added a state-based luxury themed `Button` component supporting loading/success/error statuses and click locks to prevent double-submissions.
  - Created custom `SkeletonLoaders` mirroring Recharts line, donut, and funnel shapes, plus table rows and stat cards to eliminate blank screen jumps.
  - Developed a progress-based `FileUploader` using native browser file reading and XMLHttpRequest upload tracking (0% to 100%).
  - Upgraded Telecaller, RM, Executive, and MD dashboards to use React Query hooks, optimistic timeline transitions (Updating... -> Completed/Failed), Sonner toast feedback, and skeleton loaders.
- **House Visited Photo Verification**: Added mandatory house/location photo upload verification to the Field Executive workflow's Reached Customer (`REACHED_CUSTOMER`) transition stage. Implemented multipart file validation and magic bytes inspection on the backend, uploaded verified photos to Supabase storage `loan-documents` bucket, registered the documents as `'OTHER'` type in the database, and mapped `'OTHER'` document types to render as **"House Visit Proof"** in Executive, RM, MD, and Document Manager frontend views.
- **Strict Default Branches & Deactivated Custom Branch Creation**: Restructured branch configuration to restrict available branches exclusively to "Vijayawada" and "Hyderabad". Updated backend startup routine to verify their existence and clean up any non-default branches after safely migrating existing employees to "Vijayawada". Disabled backend custom branch insertion endpoint (returns `403 Forbidden`) and completely removed the "Add New Branch" button, modal, and associated state handlers from the MD Dashboard.
- **Telecaller District Selection to Typing Input**: Replaced the district selection dropdown/select bar in the Telecaller create/edit lead form with a text input field for manual district typing. Added strict client-side validation to require the district field when saving/submitting, initialized it to an empty string on new lead creation, and retained the input's visual alignment with other step-1 form inputs.
- **Telecaller Multiple Documents Upload**: Enabled uploading multiple documents/images per category (`LOAN_SLIP`, `KYC`, and `ADDITIONAL`) in the Telecaller Lead creation/edit form (`LeadForm.tsx`). Updated local state structures to manage dynamic arrays of uploader slots with confirmation-on-delete prompts and "+" (Add More) buttons positioned next to each category title. Enforced step-4 validation requiring at least one successfully uploaded document per category before form submission.
